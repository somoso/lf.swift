...
//
        ...
        var timing:CMSampleTimingInfo = CMSampleTimingInfo(
                duration: CMTimeMake(Int64(timestamp), 600),
                presentationTimeStamp: CMTimeMake(Int64(stream.videoTimestamp) + Int64(compositionTimeoffset), 600),
        ...
        data.withUnsafeMutableBytes { (bytes: UnsafeMutablePointer<UInt8>) -> Void in
            ...
            let st = CMSampleBufferCreate(
                    kCFAllocatorDefault, blockBuffer!, true, nil, nil, stream.mixer.videoIO.formatDescription, 1, 0, null, 1, &sampleSizes, &sampleBuffer)
            guard st == noErr else {
